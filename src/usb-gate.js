(() => {
  ...
  const passwordToggle = document.getElementById('kitcPasswordToggle');
  ...
  const connectUsb = async () => {
    ...
    try {
      ...
      manifest = existing || {
        ...
        usbId: window.crypto?.randomUUID ? window.crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2)}`,
  ...
  const unlock = async event => {
    ...
    if (!usbConnected || !usbHandle || !manifest) {
      setStatus('Connect the KITC USB first.');
      setConnectedState(false);
      return;
    }
